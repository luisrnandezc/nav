...
def generate_ai_analysis_async(instance):
    """
    Asynchronous function to generate AI analysis in the background
    """
    print(f"Starting async AI analysis for report: {instance.description[:50]}...")
    
    # Update status to PROCESSING
    instance.ai_analysis_status = 'PROCESSING'
    instance.save(update_fields=['ai_analysis_status'])
    
    # Add a small delay to ensure the report is fully saved
    time.sleep(1)
    ...
    try:
        ...
        print(f"Error in async signal: {error_msg}")
...
def generate_ai_analysis_on_save(sender, instance, created, **kwargs):
    ...
    if created or instance._state.adding:
        # This is a new record, start AI analysis in background thread
        print(f"Starting background AI analysis for new report: {instance.description[:50]}...")
        
        # Start the AI analysis in a background thread
        thread = threading.Thread(
            target=generate_ai_analysis_async,
            args=(instance,),
            daemon=True  # This ensures the thread doesn't prevent Django from shutting down
        )
        thread.start()
        
        print(f"AI analysis started in background thread. Report saved successfully!")
